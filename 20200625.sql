...
* users 테이블의 컬럼 타입을 확인하고
  red_dt 컬럼 값에 5일 뒤 날짜를 새로운 컬럼으로 표현
...
적용 방법 : 컬럼, expression [AS] 별칭명   [] 안에 생략가능
별칭을 소문자로 적용 하고 싶은 경우 : 별칭명을 쌍따옴표로 묶는다. 공백을 넣고싶은 경우도 가능
SELECT empno, ename, sal s, comm AS "commition", sal + comm AS sal_plus_comm, sal + comm AS "sal plus comm"
FROM emp;
...
java : System.out.println("test"); , java에서는 쌍따옴표로 문자열을 표기한다.
sql : 'test', sql에서는 따옴표로 문자열을 표기
...
USER테이블의 userid 컬럼과 usernm 컬럼을 결합
...
비교값이 시작값고 종료값을 포함하여 사이에 있으면 참으로 인식
...
IN연산자를 사용하지 않고 OR연산을 통해서도 동일한 결과를 조회하는 SQL 작성 가능
SELECT *
FROM emp
