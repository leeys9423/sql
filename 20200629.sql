...
ROWNUM의 값을 별칭을 통해서 새로운 컬럼으로 만들고 해당 SELECT SQL을 in-line view로 만들어
...
dual 테이블 : oracle의 sys 계정에 존재하는 하나의 행, 하나의 컬럼을 갖는 테이블.
...
데이트 표현하는 방법
1. 데이트 리터럴 : NLS_SESSION_PARATER 설정에 따르기 때문에
