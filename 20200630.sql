...
(활용도 : *** ) NEXT_DAY(date1, 주간요일(1~7)) : date1 이후에 등장하는 첫번째 주간요일의 날짜 변환
                                               ex) 20200630 이후에 등장하는 토요일(7)은 언제인가? 
                                                   ===> 20200704
(활용도 : ***) LAST_DAY(date1) : date1 날짜가 속한 월의 마지막 날짜 반환
                                20200605 ==> 20200630
                                모든 달의 첫번째 날짜는 1일로 정해져 있음 하지만 달의 날짜는 다른 경우가있다.
                                윤년의 경우 2월달이 29일임.
...
SELECT LAST_DAY(TO_DATE('2020/06/05', 'YYYY/MM/DD')) "6월 막날"
FROM dual;

LAST_DAY는 있는데, FIRST_DAY는 없다 ==> 모든 월의 첫번째 날짜는 동일(1일)하기 때문에 따로 없다.
FIRSY_DAY 직접 SQL로 구현
...
SELECT TO_DATE(TO_CHAR(SYSDATE, 'YYYYMM')||'01','YYYYMMDD') firstday
FROM dual;
...
조회컬럼 : ename, mgr, mgr컬럼 값이 NULL이면 111로 치환한값-NULL이 아니면 mgr 컬럼값, 
          hiredate, hiredate가 NULL이면 SYSDATE로 표기- NULL이 아니면 hiredate컬럼 값
...
emp테이블에 저장된 job 컬럼의 값을 기준으로 급여(sal)를 인상시켜려고 한다.
